...
namespace ET
{
	...
	public static class GamePlayComponentSystemSystem
	{
		...
		private static async ETTask ProcessCombatUnits(
		this GamePlayComponent self,
		List<Unit> units,
		List<ChampionInfo> infos,
		List<Unit> targetUnits)
		{
			...
			for (var i = 0; i < units.Count; i++)
			{
				...
				tasks.Add(cpCombatComponent.CombatLoop(self, unit, infos[i], targetUnits));
			}
		...
		public static async ETTask CombatLoop(this GamePlayComponent self)
		{
			...
			while (true)
			{
                ...
                await self.ProcessCombatUnits(firstUnits, firstInfos, secondUnits);
                await self.ProcessCombatUnits(secondUnits, secondInfos, firstUnits);
