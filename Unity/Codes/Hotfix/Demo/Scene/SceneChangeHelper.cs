namespace ET
{
	...
	public static class SceneChangeHelper
	{
		...
		{
			await Game.EventSystem.PublishAsync(new EventType.SceneChangeStart()
			{
				ZoneScene = zoneScene,
				Name = sceneName
			});
		...
		public static async ETTask SceneChangeToChessMap(Scene zoneScene, string sceneName, long sceneInstanceId)
		{
			...
			await Game.EventSystem.PublishAsync(new EventType.SceneChangeChessMapStart()
			{
				ZoneScene = zoneScene,
				Name = sceneName
			});

			await Game.EventSystem.PublishAsync(new EventType.SceneChangeChessMapFinish()
			{
				ZoneScene = zoneScene,
				CurrentScene = currentScene
			});
			...
			zoneScene.GetComponent<SessionComponent>().Session.Send(c2GSceneReady);
		}
	}
}
