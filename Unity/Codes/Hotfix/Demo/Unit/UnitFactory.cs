...
namespace ET
{
	public static class UnitFactory
	{
		public static Unit Create(Scene currentScene, UnitInfo unitInfo)
		{
			...
			switch (unit.Type)
			{
				case UnitType.Monster:
					...
					numericComponent.Set(NumericType.Speed, 10);
				...
				case UnitType.Player:
				{
					...
					{
						BuffComponent buffC = combatU.GetComponent<BuffComponent>();
		...
		public static Unit CreateSkillCollider(Scene currentScene, int configId, Vector3 pos, Quaternion rota, SkillPara para)
		{
			...
			unit.Position = pos;
			unit.Rotation = rota;
			SkillJudgeConfig collider = SkillJudgeConfigCategory.Instance.Get(configId);
			if (collider.ColliderType == ColliderType.Target) //朝指定位置方向飞行碰撞体
			{
				NumericComponent numc = unit.AddComponent<NumericComponent>();
