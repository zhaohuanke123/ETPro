...
namespace ET
{
	...
	public class UIMessageWindowOnCreateSystem: OnCreateSystem<UIMessageWindow>
	{
		public override void OnCreate(UIMessageWindow self)
		{
			...
			self.FastBtn.GetGameObject().SetActive(false);
			...
			self.RecordBtn.SetOnClick(() => { self.OnRecordBtnClick().Coroutine(); });
			self.RecordBtn.GetGameObject().SetActive(false);
	...
	public static class UIMessageWindowSystem
	{
		...
		public static async ETTask OnRecordBtnClick(this UIMessageWindow self)
		{
			...
			GalGameEngineComponent.Instance.AutoPlay,
			GalGameEngineComponent.Instance.ReviewItems,
