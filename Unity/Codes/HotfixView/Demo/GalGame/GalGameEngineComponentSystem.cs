...
namespace ET
{
    ...
    public static class GalGameEngineComponentSystem
    {
        ...
        /// <returns></returns>
        public static async ETTask ShowMessage(this GalGameEngineComponent self, string text, string name, string windowType, string pageCtrl,
        string voice, long waitTime = 1000)
        {
            ...
            {
                UIMessageWindow win =
                        await UIManagerComponent.Instance.OpenWindow<UIMessageWindow, float, long>(UIMessageWindow.PrefabPath, self.Speed, waitTime);
            ...
            {
                var win = await UIManagerComponent.Instance.OpenWindow<UIBaseMessageWindow, float, long>(UIBaseMessageWindow.UIMessageWindowFull,
                    self.Speed,
                    waitTime);
                await win.SetContent(text, clear: refresh);
            }
            ...
            {
                var win = await UIManagerComponent.Instance.OpenWindow<UIBaseMessageWindow, float, long>(UIBaseMessageWindow.UIMessageWindowMiddle,
                    self.Speed,
                    waitTime);
                await win.SetContent(text, clear: refresh);
            }
        ...
        public static void AppendReviewItem(this GalGameEngineComponent self, string name, string content, bool isContinue = false)
        {
            ...
            {
                self.ReviewItems.Add(new GalGameEngineComponent.ReviewItem { Name = name, Content = content, Continue = isContinue, });
            }
            else
            {
