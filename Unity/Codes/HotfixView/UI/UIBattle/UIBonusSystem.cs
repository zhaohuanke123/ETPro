...
namespace ET
{
	...
	public class UIBonusOnCreateSystem: OnCreateSystem<UIBonus, int>
	{
		...
		{
			self.index = index;
			self.icon = self.AddUIComponent<UIImage>("IconBox/ICON");
			// self.name = self.AddUIComponent<UIText>("name");
			// self.count = self.AddUIComponent<UIText>("count");
			self.activeBg = self.AddUIComponent<UIImage>("IconBox/SPR_Frame");
			self.SetActive(false);
			self.slider = self.AddUIComponent<UISlider>("Slider");
		}
	}
	...
	public static class UIBonusSystem
	{
		public static void SetBonus(this UIBonus self, int bonusId, int count)
		{
			if (bonusId != -1)
			{
				...
				self.slider.SetMaxValue(championBonusConfig.championCount);
