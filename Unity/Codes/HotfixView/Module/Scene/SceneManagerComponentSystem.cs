    ...
    public static class SceneManagerComponentSystem
    {
        ...
        SceneLoadComponent slc = null)
        {
            ...
            if (needclean)
            {
                ...
                {
                    string path = self.ScenesChangeIgnoreClean[i];
                    GameObject go = GameObjectPoolComponent.Instance.GetCachedGoWithPath(path);
                    if (go != null)
                    {
                        gos.Add(go);
                    }
            ...
            AsyncOperation res = Resources.UnloadUnusedAssets();
            while (!res.isDone)
            {
                await Game.WaitFrameFinish();
            }
        ...
        {
            if (self.SceneConfigs.TryGetValue(name, out SceneConfig res))
            {
                return res;
            }
